...
---
	...
	1. [单例模式定义：](#单例模式定义)
		...
		2. [线程安全的问题](#线程安全的问题)
		3. [单例模式的选择](#单例模式的选择)
		4. [Spring中的单例模式详细请见同目录下的spring's singleton](#spring中的单例模式详细请见同目录下的springs-singleton)
	2. [实现单例模式的方式](#实现单例模式的方式)
		5. [饿汉式单例（立即加载方式）](#饿汉式单例立即加载方式)
		6. [懒汉式单例（延迟加载方式）](#懒汉式单例延迟加载方式)
		7. [静态内部类实现](#静态内部类实现)
		8. [static静态代码块实现](#static静态代码块实现)
		9. [内部枚举类实现](#内部枚举类实现)
...
````
    ...
    // 静态工厂方法
   	 	public static Singleton1 getInstance() {//别的类可以通过这个构造方法来进行访问
        return single;
    	}
	}
````
...
&emsp;&emsp;在方法上加synchronized同步锁或是用同步代码块对类加同步锁，此种方式虽然解决了多个实例对象问题，但是该方式运行效率却很低下，下一个线程想要获取对象，就必须等待上一个线程释放锁之后，才可以继续运行。
...
&emsp;&emsp;使用双重检查进一步做了优化，可以避免整个方法被锁，只对需要锁的代码部分加锁，可以提高执行效率。
...
&emsp;&emsp;以上就是本文要介绍的所有单例模式的理解和实现，相信这篇文章能让大家更清楚的理解单例模式，希望大家有问题可以探讨，多多指教！
