...
&emsp;&emsp;单例模式确保了某个类只有一个实例，而且是由自己所创建的，并向这个系统提供了这个实例，减少了内存的占用，一般工具类、创建一个对象需要消耗大量资源的情况下。如果长时间不使用，也会被垃圾回收器回收，造成程序出现问题。
&emsp;&emsp;单例模式肯定会存在变量的线程安全问题，因为单例模式的方法是static的，所以在类加载的时候创建实例，此时，可能有多个线程同时通过类名.getIntance方法调用，就会出现线程安全问题，所以，我们可以通过静态代码块的方式进行加锁处理，保证线程安全，但是这又会托慢速度，所以，可以通过双重检查（判断），来进行同步锁的细粒化，进而提高执行效率。
&emsp;&emsp;也可以通过静态代码块进行单例模式的实例化，虽然通过这种方式可以实现线程安全，但是但遇到序列化对象时，会产生多例。写了一个小的demo，确实是不一样的，因为没有同步序列化与反序列化，反序列化后的实例是重新生成的。可以使用object中的readResolve方法在返回反序列化之前替换、解析流中读出来的对象
&emsp;&emsp;还可以通过内部类的方式进行单例模式的实例化，利用枚举也可以，但是这个还不是太了解。
