...
![隐藏某些字段](https://www.github.com/only-wjt/images/raw/master/小书匠/根据路程节点隐藏某些字段.png)
			 ...
			 //市内交通报销隐藏
			 $("#_GRID_COMMON")[0].style.display="none";
			 //差旅报销隐藏
			 $("#_DAILY_PAYMENT")[0].style.display="none";
			 //招待费隐藏
			 $("#_ENTERTAIN")[0].style.display="none";
			
			//零星费用报销隐藏
			$("#_GRID_ENT")[0].style.display="none";	
       		//出差地点和事由隐藏
       		$("[name='TRIP_STARTDATE']").hide();
            $("[name='TRIP_ENDDATE']").hide();
            $("[name='PAY_CONTENT']").hide();
       		uiVisible("TRIP_STARTDATE", false);
            uiVisible("TRIP_ENDDATE", false);
            uiVisible("PAY_CONTENT", false);
			uiRequiredClass("TRIP_STARTDATE", false);
			uiRequiredClass("TRIP_ENDDATE", false);
			uiRequiredClass("PAY_CONTENT", false);
       		//当业务类型不为招待费时，隐藏招待费关联任务字段
       		$("[name='ENTERTAIN_TASK']").hide();
       		$("#task").hide();
       		uiVisible("ENTERTAIN_TASK", false);
       		uiRequiredClass("ENTERTAIN_TASK", false);