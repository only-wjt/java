...
//		builderSQL.append("SELECT COUNT(1) FROM (SELECT DISTINCT ORGANID FROM ").append(DZZTableModel.getT_DZZ_INFO())
//				.append(" WHERE INSTR('").append(organIdStr.toString().substring(0, organIdStr.length()-1))
//				.append("',ORGANID)>0").append(" START WITH ORGANID = '").append(dzzIDStr.toString().substring(0, dzzIDStr.length()-1))
//				.append("' CONNECT BY PRIOR ORGANID = PARENTID AND OPERATETYPE<>3)");
