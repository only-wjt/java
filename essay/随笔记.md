...
## 发票管理出了刚录入，其他状态不能修改
	...
	由@form(BO_EU_FM_EXPENSE_RESEARCH,USER_NAME)【@date】发起的科研费申请流程，科研费金额为@form(BO_EU_FM_EXPENSE_RESEARCH,AMOUNT)元
			...
			String[] billTypes = statistricType.split(",");
			for (String billType : billTypes) {
				if(StringUtils.equals(billType, "1")) {//科研经费
					ProcessInstance processInstance1=null;
					try {
						processInstance1 = SDK.getProcessAPI().createProcessInstance(
								"obj_096de4e5140d420085dd1c2b86736fee", "admin" , "科研费流程");
					} catch (Exception e) {
						throw new AWSAPIException("流程启动失败");
					}
					
					SDK.getBOAPI().create("BO_EU_FM_EXPENSE_RESEARCH", new BO().set("USER_NAME", "申请人"), processInstance1.toString(), "admin");
				}
				SDK.getProcessAPI().start(processInstance1);
			}