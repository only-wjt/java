...
## 发票管理出了刚录入，其他状态不能修改
	...
	所有流程标题修改
	合同收款登记付款单位根据合同在合同档案中选择出来，显示成甲方-乙方-丙方-丁方的形式
	财务初审和申请人节点进行是否有没有的校验以及预算库中对应科目的金额计算(合同收款登记中只对非合同收款的情况下进行预算金额的校验)
	
	
	SELECT a.CAR_BH, a.PLATE_NUMBER, a.CAR_LICENSE, a.CAR_TYPE, a.CAR_STATUS, b.CNNAME, c.CN2 FROM BO_EU_BASE_CAR a LEFT JOIN ( SELECT ITEMNO, CNNAME FROM BO_ACT_DICT_KV_ITEM WHERE DICTKEY = 'CAR_TYPE' ) b ON a.CAR_TYPE = b.ITEMNO LEFT JOIN ( SELECT ITEMNO, CNNAME CN2 FROM BO_ACT_DICT_KV_ITEM WHERE DICTKEY = 'CAR_STATUS' ) c ON a.CAR_STATUS = c.ITEMNO WHERE a.CAR_STATUS = '1' OR a.CAR_STATUS = '7'